...
/**
 * Earth Observation (EO) integration test scenario for Algorand.
 *
 * This test demonstrates the complete EO workflow using:
 * - Algorand blockchain for anchoring (testnet)
 * - In-memory DID method (did:key) for DID operations
 * - Integrity chain verification (Blockchain → VC → Linkset → Artifacts)
 *
 * This test uses Algorand testnet for blockchain anchoring, providing real
 * blockchain integration while remaining free and fast for testing.
 */
class AlgorandEoIntegrationTest {
    ...
    fun `end-to-end EO integrity chain verification with Algorand`() = runBlocking {
        ...
        // Step 5: Anchor VC digest to Algorand blockchain (testnet)
        ...
        println("EO Integrity chain verification successful with Algorand!")
    ...
    fun `test Algorand blockchain anchoring for EO dataset`() = runBlocking {
        ...
        println("Successfully anchored and retrieved EO dataset digest from Algorand testnet")
        println("Transaction Hash: ${anchorResult.ref.txHash}")
    }
}
