...
class OptimismBlockchainAnchorClientTest {
    ...
    @Test
    fun `legacy contract option key flows into the anchor ref contract`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `contractAddress option key still flows into the anchor ref contract`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `legacy contract key wins over contractAddress when both are set`() =
        runBlocking<Unit> {
            val client =
                client(
                    mapOf(
    ...
    @Test
    fun `no contract option leaves the anchor ref contract unset`() =
        runBlocking<Unit> {
            val result = client().writePayload(payload)
