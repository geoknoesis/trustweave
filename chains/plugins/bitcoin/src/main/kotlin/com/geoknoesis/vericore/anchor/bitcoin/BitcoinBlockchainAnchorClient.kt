package com.geoknoesis.vericore.anchor.bitcoin

import com.geoknoesis.vericore.anchor.*
import com.geoknoesis.vericore.anchor.exceptions.BlockchainTransactionException
...
class BitcoinBlockchainAnchorClient(
    ...
    override protected suspend fun submitTransactionToBlockchain(
        ...
        val signedTxHex = signedTxJson["hex"]?.jsonPrimitive?.content
            ?: throw VeriCoreException("Failed to sign transaction")
        
        // Check if transaction is complete
        val complete = signedTxJson["complete"]?.jsonPrimitive?.boolean ?: false
        if (!complete) {
            throw VeriCoreException("Transaction signing incomplete")
        }
    ...
    private suspend fun rpcCall(method: String, params: List<Any?>): String = withContext(Dispatchers.IO) {
        ...
        if (!response.isSuccessful) {
            throw VeriCoreException("Bitcoin RPC call failed: ${response.code} - ${response.message}")
        }
        
        val jsonResponse = Json.parseToJsonElement(responseBody ?: "{}").jsonObject
        val error = jsonResponse["error"]
        if (error != null && error !is JsonNull) {
            throw VeriCoreException("Bitcoin RPC error: ${error.jsonObject["message"]?.jsonPrimitive?.content}")
        }
        
        jsonResponse["result"]?.jsonPrimitive?.content
            ?: throw VeriCoreException("No result in Bitcoin RPC response")
    }
