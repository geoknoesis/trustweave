 ...
 *
 * Note: the `.toException()` helper is provided as an extension function in
 * `org.trustweave.core.util.ValidationExtensions` (JVM-only module `:common`) because it
 * references `TrustWeaveException` which remains JVM-only for now.
 *
 * **Example:**
 * ```kotlin
