package com.geoknoesis.vericore.contract
...
class DefaultSmartContractService(
    ...
    ): Result<SmartContract> = vericoreCatching {
        // Validate request
        val validation = ContractValidator.validateDraftRequest(request)
        if (!validation.isValid()) {
            throw com.geoknoesis.vericore.core.InvalidOperationException(
    ...
    override suspend fun anchorContract(
        ...
        chainId: String
    ): Result<AnchorRef> = vericoreCatching {
    ...
    override suspend fun bindContract(
        ...
        issuerKeyId: String,
        chainId: String
    ): Result<BoundContract> = vericoreCatching {
        val contract = contracts[contractId]
            ?: throw com.geoknoesis.vericore.core.NotFoundException("Contract not found: $contractId")
    ...
    ): Result<ExecutionResult> = vericoreCatching {
    ...
    override suspend fun getContract(contractId: String): Result<SmartContract> = vericoreCatching {
    ...
    override suspend fun verifyContract(
        ...
        val contract = contracts.values.firstOrNull { it.credentialId == credentialId }
            ?: throw com.geoknoesis.vericore.core.NotFoundException(
    ...
    private suspend fun updateContract(contract: SmartContract): Result<SmartContract> = vericoreCatching {
