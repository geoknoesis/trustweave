...
import org.trustweave.credential.exchange.response.ProofExchangeResponse
...
import org.trustweave.credential.identifiers.IssueId
import org.trustweave.credential.identifiers.OfferId
import org.trustweave.credential.identifiers.PresentationId
...
internal class DefaultExchangeService(
    ...
    private fun Exception.toExchangeFailure(operation: String): ExchangeResult.Failure =
        when (this) {
            is IllegalArgumentException ->
                ExchangeResult.Failure.InvalidRequest(
                    field = "request",
            ...
            is NoSuchElementException ->
                ExchangeResult.Failure.MessageNotFound(
                    messageId = "",
            ...
            is IOException,
            ->
                ExchangeResult.Failure.NetworkError(
                    reason =
                        when (this) {
    ...
    override suspend fun offer(request: ExchangeRequest.Offer): ExchangeResult<ExchangeResponse.Offer> {
        ...
        val protocol =
            protocolRegistry.get(protocolName)
            ...
            val offerId =
                OfferId(
                    java.util.UUID
                        .randomUUID()
                        .toString(),
                ) // Generate or extract from envelope
    ...
    override fun supports(protocolName: ExchangeProtocolName): Boolean = protocolRegistry.isRegistered(protocolName)

    override fun supports(
        protocolName: ExchangeProtocolName,
        operation: ExchangeOperation,
    ): Boolean = protocolRegistry.supports(protocolName, operation)

    override fun supports(operation: ExchangeOperation): Boolean = protocolRegistry.getAll().values.any { it.supports(operation) }
    ...
    override fun supportedProtocols(): List<ExchangeProtocolName> = protocolRegistry.getSupportedProtocols()

    override fun getCapabilities(protocolName: ExchangeProtocolName): ExchangeProtocolCapabilities? =
        protocolRegistry.getCapabilities(protocolName)
}
