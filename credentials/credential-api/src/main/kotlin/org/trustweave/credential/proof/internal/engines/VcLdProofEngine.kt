...
internal class VcLdProofEngine(
    ...
    override suspend fun issue(request: IssuanceRequest): VerifiableCredential {
        ...
        val proofSuiteContext =
            if (requestedSuite == CredentialConstants.ProofTypes.JSON_WEB_SIGNATURE_2020) {
        ...
        val verificationMethodIdString =
            request.issuerKeyId?.value
                ?: throw IllegalArgumentException("issuerKeyId is required")

        val verificationMethod =
            if (verificationMethodIdString.contains("#") && verificationMethodIdString.startsWith("did:")) {
    ...
    override suspend fun verify(
        ...
        // Validate proof type
        val proof =
            validateProofType(credential) ?: return createInvalidProofResult(
                credential,
        ...
        return try {
            ...
            val verificationMethod =
                vmResolver.resolve(issuerIri, proof.verificationMethod, expectedPurpose)
                    ?: return createInvalidIssuerResult(credential, issuerIri)

            // Canonicalize document
            val canonical =
                canonicalizationPort.canonicalize(
                    JsonLdDocumentBuilder.buildWithoutProof(credential),
                )
            logger.debug(
                "Verifying VC-LD credential: canonicalLength={}, verificationMethod={}, publicKeyJwkPresent={}",
                canonical.length,
    ...
    private fun createInvalidIssuerResult(
        ...
        VerificationResult.Invalid.InvalidIssuer(
            ...
            issuerIri = issuerIri,
            reason =
                "Could not resolve issuer IRI, find the proof's verification method, or the " +
    ...
    override suspend fun createPresentation(
        ...
        val credentialsToInclude =
            if (disclosedClaims != null && disclosedClaims.isNotEmpty()) {
        ...
        val holder =
            credentialsToInclude.first().credentialSubject.id
    ...
    private suspend fun signPayload(
        ...
        val signer =
            getSignerFunction() ?: run {
                val kms =
                    getKms() ?: throw IllegalStateException(
     ...
     */
    private suspend fun signPayloadAsJws(
        payload: ByteArray,
        keyId: String,
        algorithm: String,
    ): String {
        ...
        val signingInput =
            "${header.toBase64URL()}.${Base64URL.encode(payload)}"
        ...
        val jwsSignatureBytes =
            if (ProofEngineUtils.isEcdsaJwsAlgorithm(jwsAlgorithm)) {
        ...
        { data: ByteArray, kid: String ->
            ...
            when (signResult) {
                ...
                is SignResult.Failure.Error -> throw IllegalStateException(
                    "Failed to sign VC-LD document: ${signResult.reason}",
                    signResult.cause,
                )
            }
        }
