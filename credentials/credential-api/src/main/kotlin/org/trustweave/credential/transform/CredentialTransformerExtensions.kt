 ...
 */
suspend fun VerifiableCredential.toJwt(): String = defaultTransformer.toJwt(this)
 ...
 */
suspend fun VerifiableCredential.toJsonLd(): JsonObject = defaultTransformer.toJsonLd(this)
 ...
 */
suspend fun VerifiableCredential.toCbor(): ByteArray = defaultTransformer.toCbor(this)
...
suspend fun JsonObject.toCredential(): VerifiableCredential {
    // This is where an untrusted JSON-LD document becomes a credential the rest of the library
    // treats as a model object — the last point at which its size is still a property of the input
    // rather than of whatever it was parsed into. SecurityConstants.MAX_CREDENTIAL_SIZE_BYTES had
    // no enforcement anywhere before this, which made it read as a limit while being only a number.
    val sizeBytes = this.toString().toByteArray(Charsets.UTF_8).size
    require(sizeBytes <= SecurityConstants.MAX_CREDENTIAL_SIZE_BYTES) {
        "Credential document exceeds the maximum of " +
            "${SecurityConstants.MAX_CREDENTIAL_SIZE_BYTES} bytes: $sizeBytes bytes"
    }
 ...
 */
suspend fun ByteArray.fromCbor(): VerifiableCredential = defaultTransformer.fromCbor(this)
 ...
 */
suspend fun VerifiableCredential.roundTripCbor(): VerifiableCredential = this.toCbor().fromCbor()
 ...
 */
suspend fun VerifiableCredential.roundTripJsonLd(): VerifiableCredential = this.toJsonLd().toCredential()
