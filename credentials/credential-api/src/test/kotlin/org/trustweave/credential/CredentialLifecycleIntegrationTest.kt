...
class CredentialLifecycleIntegrationTest {
    companion object {
        ...
        private fun testClaimsProofOptions() =
            org.trustweave.credential.proof.proofOptions {
                option(
    ...
    @Test
    fun `test full credential lifecycle - issue and verify`() =
        runBlocking<Unit> {
            ...
            // Get issuer key ID
            val issuerKeyId =
                issuerDocument.verificationMethod.firstOrNull()?.id
    ...
    @Test
    fun `test credential lifecycle with expiration check`() =
        runBlocking<Unit> {
            // Setup
            val kms: KeyManagementService = InMemoryKeyManagementService()
            val didMethod = DidKeyMockMethod(kms)
            ...
            // Should be expired
            assertTrue(
                verificationResult is VerificationResult.Invalid.Expired,
                "Credential should be expired, got: ${verificationResult::class.simpleName}",
            )
        }

    @Test
    fun `test batch verification`() =
        runBlocking<Unit> {
            // Setup
            val kms: KeyManagementService = InMemoryKeyManagementService()
            val didMethod = DidKeyMockMethod(kms)
                        ...
                        val reason =
                            when (result) {
                                ...
                                is VerificationResult.Invalid.Revoked ->
                                    "Revoked at: ${result.revokedAt}, reason: ${result.revocationReason}"
                                ...
                                is VerificationResult.Invalid.SchemaValidationFailed ->
                                    "Schema validation failed: ${result.validationErrors.joinToString()}"
    ...
    @Test
    fun `test issuance fails with unsupported format`() =
        runBlocking<Unit> {
            // Setup
            val kms: KeyManagementService = InMemoryKeyManagementService()
            val didMethod = DidKeyMockMethod(kms)
            ...
            assertTrue(
                result is IssuanceResult.Failure.UnsupportedFormat,
                "Should fail with UnsupportedFormat, got: ${result::class.simpleName}",
            )
        }

    @Test
    fun `test verification fails with invalid issuer DID`() =
        runBlocking<Unit> {
            // Setup
            val kms: KeyManagementService = InMemoryKeyManagementService()
            val didMethod = DidKeyMockMethod(kms)
            ...
            assertTrue(
                verificationResult is VerificationResult.Invalid.InvalidIssuer,
                "Should fail with InvalidIssuer, got: ${verificationResult::class.simpleName}",
            )
        }

    @Test
    fun `test verification fails with credential missing proof`() =
        runBlocking<Unit> {
            // Setup
            val kms: KeyManagementService = InMemoryKeyManagementService()
            val didMethod = DidKeyMockMethod(kms)
            ...
            assertTrue(
                verificationResult is VerificationResult.Invalid.InvalidProof,
                "Should fail with InvalidProof, got: ${verificationResult::class.simpleName}",
            )
        }

    @Test
    fun `test issuance fails with too many claims`() =
        runBlocking<Unit> {
            // Setup
            val kms: KeyManagementService = InMemoryKeyManagementService()
            val didMethod = DidKeyMockMethod(kms)
            ...
            assertTrue(
                failure.reason.contains("exceeds maximum claims count"),
