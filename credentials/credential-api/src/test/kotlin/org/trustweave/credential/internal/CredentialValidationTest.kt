...
class CredentialValidationTest {
    ...
    @Test
    fun `test validateTrust with no trust policy`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `test validateTrust with trusted issuer`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `test validateTrust with untrusted issuer`() =
        runBlocking<Unit> {
            val issuerDid = Did("did:key:z6MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnnEGta2doK")
    ...
    @Test
    fun `test validateTrust with non-DID issuer`() =
        runBlocking<Unit> {
    ...
    private fun createTestCredential(issuer: Issuer = Issuer.IriIssuer(Iri("did:key:test"))): VerifiableCredential =
        VerifiableCredential(
            context = listOf(CredentialConstants.VcContexts.VC_1_1),
            type = listOf(CredentialType.fromString("VerifiableCredential")),
            issuer = issuer,
