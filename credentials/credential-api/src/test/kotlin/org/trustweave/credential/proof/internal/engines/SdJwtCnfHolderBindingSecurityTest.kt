...
class SdJwtCnfHolderBindingSecurityTest {
    private class TestRig {
        ...
        suspend fun craftKbJwt(
            ...
            val payloadJson =
                """{"iat":${Clock.System.now().epochSeconds},"nonce":"$challenge",""" +
    ...
    @Test
    fun `issuance embeds cnf kid bound to the holder DID`() =
        runBlocking<Unit> {
            val rig = TestRig()
    ...
    @Test
    fun `cnf round-trip presentation verifies without opting into enforceHolderBinding`() =
        runBlocking<Unit> {
            val rig = TestRig()
            val credential = rig.issue()
    ...
    fun `kb-jwt signed by a different DID than cnf is rejected even when envelope holder matches the attacker`() =
        runBlocking {
            ...
            assertTrue(
                (invalid as? VerificationResult.Invalid.InvalidProof)
                    ?.reason
    ...
    @Test
    fun `envelope holder differing from cnf DID is rejected even with a genuine holder kb-jwt`() =
        runBlocking<Unit> {
            val rig = TestRig()
            val credential = rig.issue()
            val presentation = rig.present(credential)
    ...
    @Test
    fun `cnf binding is enforced for any presentation proof format via verifyCnfHolderBinding`() =
        runBlocking<Unit> {
            val rig = TestRig()
            val credential = rig.issue()
            ...
            val hijacked =
                org.trustweave.credential.model.vc.VerifiablePresentation(
    ...
    @Test
    fun `legacy credential without cnf keeps envelope-holder binding behaviour`() =
        runBlocking<Unit> {
            val rig = TestRig()
            val legacyCredential = rig.issueLegacyWithoutCnf()
