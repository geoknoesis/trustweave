...
class SdJwtProofEngineSecurityTest {
    private class TestRig {
        ...
        suspend fun issue(
            claims: Map<String, JsonPrimitive> =
                mapOf(
    ...
    @Test
    fun `sd-jwt sign and verify round-trip succeeds`() =
        runBlocking<Unit> {
            val rig = TestRig()
    ...
    @Test
    fun `issuer key not listed under assertionMethod is rejected`() =
        runBlocking<Unit> {
            val rig = TestRig()
            val credential = rig.issue(validUntil = Clock.System.now().plus(1.hours))
    ...
    @Test
    fun `stripping envelope expirationDate does not bypass expired signed exp`() =
        runBlocking<Unit> {
            val rig = TestRig()
    ...
    @Test
    fun `rewriting envelope expirationDate to extend validity is rejected as tampered`() =
        runBlocking<Unit> {
            val rig = TestRig()
            val credential = rig.issue(validUntil = Clock.System.now().plus(1.hours))
    ...
    @Test
    fun `signed iss differing from envelope issuer is rejected`() =
        runBlocking<Unit> {
            val rig = TestRig()
    ...
    @Test
    fun `envelope claim value differing from disclosure value is rejected`() =
        runBlocking<Unit> {
            val rig = TestRig()
            val credential = rig.issue()
    ...
    @Test
    fun `envelope claim not backed by any disclosure is rejected`() =
        runBlocking<Unit> {
            val rig = TestRig()
            val credential = rig.issue()
    ...
    @Test
    fun `kb-jwt presentation round-trip passes full service verification`() =
        runBlocking<Unit> {
            val rig = TestRig()
            val credential = rig.issue(validUntil = Clock.System.now().plus(1.hours))
            val presentation =
    ...
    @Test
    fun `kb-jwt with wrong nonce fails verification`() =
        runBlocking<Unit> {
            val rig = TestRig()
            val credential = rig.issue()
            val presentation =
            ...
            val result =
                rig.service.verifyPresentation(
                    presentation,
                    null,
                    kbOptions(challenge = "a-different-nonce"),
                )
            assertInvalid(result, "KB-JWT nonce differing from the expected challenge must fail")
        }

    @Test
    fun `kb-jwt with wrong audience fails verification`() =
        runBlocking<Unit> {
            val rig = TestRig()
            val credential = rig.issue()
            val presentation =
            ...
            val result =
                rig.service.verifyPresentation(
                    presentation,
                    null,
                    kbOptions(domain = "attacker.example.com"),
                )
            assertInvalid(result, "KB-JWT audience differing from the expected domain must fail")
        }

    @Test
    fun `presentation without kb-jwt fails when proof verification is required`() =
        runBlocking<Unit> {
            val rig = TestRig()
            val credential = rig.issue()
            ...
            val result =
                rig.service.verifyPresentation(
                    presentation,
                    null,
                    VerificationOptions(verifyPresentationProof = true),
                )
            assertInvalid(result, "Missing presentation proof / KB-JWT must fail when required")
        }

    @Test
    fun `presentation with stripped kb-jwt fails`() =
        runBlocking<Unit> {
            val rig = TestRig()
            val credential = rig.issue()
            val presentation =
    ...
    @Test
    fun `kb-jwt bound to different disclosures fails sd_hash check`() =
        runBlocking<Unit> {
            val rig = TestRig()
            val credential = rig.issue()
            val presentation =
    ...
    @Test
    fun `kb-jwt older than the default max age is rejected`() =
        runBlocking<Unit> {
            val rig = TestRig()
            ...
            assertTrue(
                (result as VerificationResult.Invalid).errors.joinToString().contains("max age", ignoreCase = true) ||
                    (result as? VerificationResult.Invalid.InvalidProof)
                        ?.reason
    ...
    @Test
    fun `kb-jwt max age is configurable via the kbJwtMaxAge additional option`() =
        runBlocking<Unit> {
            val rig = TestRig()
