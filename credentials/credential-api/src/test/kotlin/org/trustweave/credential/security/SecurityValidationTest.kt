...
class SecurityValidationTest {
    ...
    fun `test security constants are consistent`() {
        ...
        assertTrue(
            SecurityConstants.MAX_PRESENTATION_SIZE_BYTES >= SecurityConstants.MAX_CREDENTIAL_SIZE_BYTES,
            "Presentation size limit should be >= credential size limit",
        )
        assertTrue(
            SecurityConstants.MAX_CANONICALIZED_DOCUMENT_SIZE_BYTES >= SecurityConstants.MAX_CREDENTIAL_SIZE_BYTES,
            "Canonicalized document size limit should be >= credential size limit",
        )
        assertTrue(
            SecurityConstants.MAX_DID_LENGTH >= 100,
            "DID length limit should be at least 100 characters",
        )
        assertTrue(
            SecurityConstants.MAX_CREDENTIAL_ID_LENGTH >= 100,
