...
class ExchangeExceptionTest {
    ...
    fun `test context includes all non-null values`() {
        val exception = DidCommException.EncryptionFailed(
        ...
        assertTrue(exception.context.containsKey("fromDid"))
        assertTrue(exception.context.containsKey("toDid"))
        // cause is not in context (it's a separate property)
    }
}
