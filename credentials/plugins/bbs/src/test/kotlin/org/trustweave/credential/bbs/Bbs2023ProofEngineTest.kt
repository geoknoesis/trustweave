...
class Bbs2023ProofEngineTest {
    @BeforeTest
    fun enableEmulation() {
        // BbsCryptoSuite is an HMAC emulation with no authenticity and is disabled by default;
        // these tests exercise the engine, so they switch it on deliberately.
    ...
    private fun resolverFor(vararg documents: DidDocument): DidResolver =
        DidResolver { did ->
            documents
                .firstOrNull { it.id.value == did.value }
    ...
    @Test
    fun `issue returns credential with DataIntegrityProof`() =
        runTest {
    ...
    @Test
    fun `issue credential has correct issuer and subject`() =
        runTest {
    ...
    @Test
    fun `issue with no configured key pair fails closed with ConfigException`() =
        runTest {
    ...
    @Test
    fun `issued credential preserves all claims`() =
        runTest {
    ...
    @Test
    fun `verify returns Valid for freshly issued credential with key resolved from issuer DID document`() =
        runTest {
    ...
    @Test
    fun `verify returns Valid when issuer DID document carries the key as publicKeyJwk`() =
        runTest {
    ...
    @Test
    fun `verify returns InvalidProof for wrong format credential`() =
        runTest {
            ...
            val vc =
                ProofEngineTestData.createMinimalCredential(
                    format = ProofSuiteId.BBS_2023,
                    issuerDid = issuerDid,
    ...
    @Test
    fun `verify returns InvalidProof for tampered claim`() =
        runTest {
            ...
            val tamperedClaims =
                vc.credentialSubject.claims.toMutableMap().also {
    ...
    @Test
    fun `verify returns InvalidProof when cryptosuite is wrong`() =
        runTest {
    ...
    @Test
    fun `verify fails when proof verificationMethod is not rooted in the issuer DID`() =
        runTest {
    ...
    @Test
    fun `verify fails when issuer DID document does not contain the verification method`() =
        runTest {
            ...
            val result = engine.verify(forged, VerificationOptions())
            val invalid =
                assertIs<VerificationResult.Invalid.InvalidIssuer>(
                    result,
    ...
    @Test
    fun `verify fails when verification method is not referenced by assertionMethod`() =
        runTest {
    ...
    @Test
    fun `verify fails closed when no DID resolver is configured`() =
        runTest {
    ...
    @Test
    fun `verify fails when issuer DID does not resolve`() =
        runTest {
    ...
    @Test
    fun `verify rejects legacy proofs embedding the public key in the verificationMethod fragment`() =
        runTest {
    ...
    @Test
    fun `claims ambiguous under legacy key=value encoding do not cross-verify`() =
        runTest {
    ...
    @Test
    fun `createPresentation fails fast because the BBS emulation cannot derive real proofs`() =
        runTest {
    ...
    @Test
    fun `verify rejects bbs-2023-derived proofs with an explicit unsupported reason`() =
        runTest {
    ...
    @Test
    fun `Bbs2023ProofEngineProvider passes keyPair and didResolver through options`() =
        runTest {
