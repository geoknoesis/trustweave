...
class EncryptedFileLocalKeyStore(
    ...
    private fun parseSecretFromJson(jsonElement: JsonElement): Secret {
        ...
        // Parse type
        val typeStr = obj["type"]?.jsonPrimitive?.content ?: "JsonWebKey2020"
        val type = when (typeStr) {
            "JsonWebKey2020" -> Secret.Type.JSON_WEB_KEY_2020
            else -> Secret.Type.JSON_WEB_KEY_2020
        }
    ...
    private fun secretToJson(secret: Secret): JsonObject {
        return buildJsonObject {
            put("id", secret.id)
            put("type", secret.type.toString())
            put("privateKeyJwk", buildJsonObject {
                secret.privateKeyJwk?.forEach { (key, value) ->
                    when (value) {
