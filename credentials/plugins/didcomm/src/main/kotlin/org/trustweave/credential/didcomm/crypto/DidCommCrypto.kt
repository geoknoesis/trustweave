...
import org.trustweave.credential.didcomm.models.DidCommProtectedHeaders
...
class DidCommCrypto(
    ...
    override suspend fun encrypt(
        ...
        try {
            ...
            val senderKeyHandle = when (senderKeyResult) {
                is org.trustweave.kms.results.GetPublicKeyResult.Success -> senderKeyResult.keyHandle
                is org.trustweave.kms.results.GetPublicKeyResult.Failure.KeyNotFound -> throw IllegalArgumentException("Sender key not found: ${senderKeyResult.keyId.value}")
                is org.trustweave.kms.results.GetPublicKeyResult.Failure.Error -> throw IllegalArgumentException("Failed to get sender key: ${senderKeyResult.reason}")
            }
    ...
    override suspend fun decrypt(
        ...
        try {
            ...
            val recipientKeyResult = kms.getPublicKey(org.trustweave.core.identifiers.KeyId(recipientKeyId))
            val recipientKeyHandle = when (recipientKeyResult) {
                is org.trustweave.kms.results.GetPublicKeyResult.Success -> recipientKeyResult.keyHandle
                is org.trustweave.kms.results.GetPublicKeyResult.Failure.KeyNotFound -> throw IllegalArgumentException("Recipient key not found: ${recipientKeyResult.keyId.value}")
                is org.trustweave.kms.results.GetPublicKeyResult.Failure.Error -> throw IllegalArgumentException("Failed to get recipient key: ${recipientKeyResult.reason}")
            }
