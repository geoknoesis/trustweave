...
class Oidc4VciService(
     ...
     */
    private class FreshNonceRequired(
        val cNonce: String,
        reason: String,
    ...
    suspend fun createCredentialRequest(
        ...
        withContext(Dispatchers.IO) {
            ...
            val issuerMetadata =
                metadata ?: throw Oidc4VciException.MetadataFetchFailed(
            ...
            val tokenResponse =
                when {
                    ...
                    preAuthorizedCode != null ->
                        exchangePreAuthorizedCodeForToken(
                ...
                try {
                    ...
                    try {
                        ...
                        throw Oidc4VciException.CredentialRequestFailed(
                            reason =
                                "Credential endpoint rejected the proof of possession again " +
    ...
    private suspend fun verifyIssuedCredential(
        ...
        val service =
            verifier ?: throw Oidc4VciException.CredentialVerificationFailed(
        ...
        if (parsed.issuer.id.value != expectedIssuer) {
            throw Oidc4VciException.CredentialVerificationFailed(
                reason =
                    "credential issuer ${parsed.issuer.id.value} does not match " +
        ...
        if (parsed.credentialSubject.id?.value != holderDid) {
            throw Oidc4VciException.CredentialVerificationFailed(
                reason =
                    "credential subject ${parsed.credentialSubject.id?.value} " +
        ...
        return try {
            when (raw) {
                ...
                is JsonPrimitive -> throw Oidc4VciException.CredentialVerificationFailed(
                    reason =
                        "compact (JWT / SD-JWT-VC) issued credentials are not yet verified; " +
     ...
     */
    suspend fun pollDeferredCredential(request: DeferredCredentialRequest): Oidc4VciIssueResult =
        withContext(Dispatchers.IO) {
            val issuerMetadata =
                metadata ?: throw Oidc4VciException.MetadataFetchFailed(
            ...
            val deferredEndpoint =
                issuerMetadata.deferredCredentialEndpoint
                    ?: throw Oidc4VciException.CredentialRequestFailed(
            ...
            val body =
                json
                    .encodeToString(JsonObject.serializer(), requestBody)
            ...
            val verifiedCredential =
                if (rawCredential == null || rawCredential is JsonNull) {
                    null
                } else {
                    val expectedIssuer =
                        request.issuerDid
                            ?: throw Oidc4VciException.CredentialVerificationFailed(
                                reason =
                                    "deferred endpoint returned a credential but the request carries " +
     ...
     */
    suspend fun issueBatchCredentials(request: BatchCredentialRequest): BatchCredentialResponse =
        withContext(Dispatchers.IO) {
            val issuerMetadata =
                metadata ?: throw Oidc4VciException.MetadataFetchFailed(
                    credentialIssuer = credentialIssuerUrl,
            ...
            val requestBody =
                json
                    .encodeToString(JsonObject.serializer(), batchBody)
                    .toRequestBody("application/json".toMediaType())
            ...
            val credentialResponsesArray =
                batchResponse["credential_responses"]?.jsonArray
                    ?: throw Oidc4VciException.CredentialRequestFailed(
            ...
            val items =
                credentialResponsesArray.map { element ->
    ...
    private fun resolveTokenEndpoint(issuerMetadata: CredentialIssuerMetadata): String {
        ...
        val authorizationServer =
            issuerMetadata.authorizationServers?.firstOrNull()
        ...
        requireHttpsOrLoopback(tokenEndpoint) { reason ->
            Oidc4VciException.TokenEndpointResolutionFailed(
                credentialIssuer = issuerMetadata.credentialIssuer,
                reason =
                    "token_endpoint '$tokenEndpoint' resolved from authorization server " +
    ...
    private fun fetchAuthorizationServerMetadata(authorizationServer: String): AuthorizationServerMetadata {
        ...
        if (asMetadata.issuer?.trimEnd('/') != authorizationServer.trimEnd('/')) {
            throw Oidc4VciException.TokenEndpointResolutionFailed(
                credentialIssuer = authorizationServer,
                reason =
                    "RFC 8414 issuer mismatch: metadata 'issuer' is " +
    ...
    private fun executeTokenRequest(
        ...
        val accessToken =
            tokenResponse["access_token"]?.jsonPrimitive?.content
                ?: throw Oidc4VciException.TokenExchangeFailed(
    ...
    private suspend fun requestCredentialFromIssuer(
        ...
        val requestBody =
            json
                .encodeToString(JsonObject.serializer(), credentialRequest)
                .toRequestBody("application/json".toMediaType())
     ...
     */
    suspend fun parseCredentialOfferUri(offerUri: String): Oidc4VciOffer =
        withContext(Dispatchers.IO) {
            val queryParams = parseQueryParameters(offerUri.substringAfter("?", ""))

            val offerJsonString =
                queryParams["credential_offer"]
            ...
            val credentialIssuer =
                offerJson["credential_issuer"]?.jsonPrimitive?.contentOrNull
                    ?: throw Oidc4VciException.OfferParseFailed(
                        offerUri = offerUri,
            ...
            if (credentialIssuer.trimEnd('/') != credentialIssuerUrl.trimEnd('/')) {
                throw Oidc4VciException.OfferParseFailed(
                    offerUri = offerUri,
                    reason =
                        "credential_offer is for issuer '$credentialIssuer' but this service " +
            ...
            val credentialConfigurationIds =
                offerJson["credential_configuration_ids"]
                    ?.jsonArray
            ...
            val txCode =
                (grantsJson?.get(PRE_AUTHORIZED_CODE_GRANT_TYPE) as? JsonObject)
                    ?.get("tx_code")
                    ?.let { element ->
                        runCatching {
     ...
     */
    suspend fun fetchCredentialIssuerMetadata(credentialIssuerUrl: String): CredentialIssuerMetadata =
        withContext(Dispatchers.IO) {
            val request =
                Request
                    .Builder()
