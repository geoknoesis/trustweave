...
class Oidc4VpServiceTest {
    ...
    @Test
    fun `test parseAuthorizationUrl with valid openid4vp URL`() =
        runBlocking {
    ...
    @Test
    fun `test createPermissionResponse with valid credentials`() =
        runBlocking {
            // Create a test credential
            val credential = createTestCredential()
            val presentableCredential =
    ...
    @Test
    fun `createPermissionResponse refuses to mint a VP token with no nonce`() {
        // The nonce is what binds a presentation to one verifier's session. Without it the signed
        // vp_token is replayable to any other verifier, so it must never be minted - OpenID4VP
        // requires nonce on the authorization request for every flow, not only under HAIP.
        val presentableCredential =
            PresentableCredential(
                credentialId = "cred-1",
        ...
        val nonceless = createTestPermissionRequest(nonce = null)

        assertFailsWith<Oidc4VpException> {
            runBlocking {
                service.createPermissionResponse(
                    permissionRequest = nonceless,
                    selectedCredentials = listOf(presentableCredential),
                    holderDid = holderDid,
                    keyId = keyId,
                )
    ...
    @Test
    fun `test submitPermissionResponse with valid response`() =
        runBlocking {
    ...
    @Test
    fun `submitPermissionResponse sends presentation_submission as a JSON form field`() =
        runBlocking {
            val requestUri = "${mockWebServer.url("/request")}"
            val responseUri = "${mockWebServer.url("/response")}"
    ...
    @Test
    fun `test fetchVerifierMetadata with valid verifier`() =
        runBlocking {
    ...
    @Test
    fun `vp_token embeds real credential JSON with request audience and EdDSA alg`() =
        runBlocking {
            val credential = createTestCredential()
            val permissionRequest = createTestPermissionRequest()
    ...
    @Test
    fun `vp_token aud falls back to response endpoint when client_id is absent`() =
        runBlocking {
            val responseUri = "${mockWebServer.url("/response")}"
            val authRequest =
                org.trustweave.credential.oidc4vp.models.AuthorizationRequest(
    ...
    @Test
    fun `presentation_submission descriptor paths align with vp_token credential order`() =
        runBlocking {
            val presentationDefinition =
                Json
                    .parseToJsonElement(
    ...
    @Test
    fun `presentation_submission is null without a presentation definition`() =
        runBlocking {
            val permissionResponse =
                service.createPermissionResponse(
                    permissionRequest = createTestPermissionRequest(),
    ...
    @Test
    fun `jwt-proof credential uses registered jwt_vc_json format identifier`() =
        runBlocking {
            val presentationDefinition =
                Json
                    .parseToJsonElement(
            ...
            val jwtCredential =
                createTestCredential().copy(
                    proof =
                        org.trustweave.credential.model.vc.CredentialProof
                            .JwtProof(jwt = "a.b.c"),
                )
    ...
    @Test
    fun `required input descriptor with no matching credential throws RequiredCredentialMissing`() =
        runBlocking {
            ...
            val exception =
                assertFailsWith<Oidc4VpException.RequiredCredentialMissing> {
                    service.createPermissionResponse(
                        permissionRequest = createTestPermissionRequest(presentationDefinition),
                        selectedCredentials =
                            listOf(
    ...
    @Test
    fun `signed request object claims take precedence and URL params do not override them`() =
        runBlocking {
