...
internal object IntegrityChecker {
    ...
    fun checkoutHashBinding(
        ...
        val txId =
            payment["transaction_id"]?.contentOrNull()
    ...
    fun l2ReferenceBinding(
        payment: JsonObject,
        checkoutDisclosureB64: String,
    ): Pair<Boolean, String> {
        val ref =
            (payment["constraints"] as? JsonArray)
                ?.mapNotNull { it as? JsonObject }
                ?.firstOrNull { it["type"]?.contentOrNull() == Constraint.Reference.TYPE }
                ?: return true to ""
        val expected =
            ref["conditional_transaction_id"]?.contentOrNull()
    ...
    fun l3CrossReference(
        l3Payment: JsonObject,
        l3Checkout: JsonObject,
    ): Pair<Boolean, String> {
        val txId =
            mandateField(l3Payment, Vct.PAYMENT_FINAL, "transaction_id")
                ?: return false to "L3a payment mandate missing transaction_id"
        val checkoutHash =
            mandateField(l3Checkout, Vct.CHECKOUT_FINAL, "checkout_hash")
