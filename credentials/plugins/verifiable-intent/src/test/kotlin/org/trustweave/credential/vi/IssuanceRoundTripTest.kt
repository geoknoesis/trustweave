...
class IssuanceRoundTripTest {
    ...
    /**
     * The ledger scopes a budget account by the L2 JWT alone, without its disclosures, so one
     * signed L2 carrying two payment-mandate disclosures with different budgets resolves to the
     * same account under different presentations. The second presentation makes the ledger raise
     * `IllegalStateException("Budget policy differs from stored mandate")` from inside `reserve`.
     * That input is presenter-controlled and the documented contract of `verifyAndReserveBudget`
     * is a [ChainVerificationResult], so no unchecked exception may escape it. A data source that
     * fails on connection reproduces the same escape path without needing a database.
     */
    @Test
    fun `ledger failures are reported as an invalid chain rather than thrown to the caller`() =
        runBlocking<Unit> {
            for (failure in listOf(IllegalStateException("policy differs"), IllegalArgumentException("bad budget"))) {
                val refusing =
                    object : javax.sql.DataSource by org.postgresql.ds.PGSimpleDataSource() {
                        override fun getConnection(): java.sql.Connection = throw failure

                        override fun getConnection(
                            username: String?,
                            password: String?,
                        ): java.sql.Connection = throw failure
                    }
                val result =
                    runAutonomous(
                        20_000,
                        ...
                        budgetLedger =
                            org.trustweave.credential.vi.verification
                                .PostgresIntentLedger(refusing),
                    )
                result.valid shouldBe false
                result.errors.joinToString() stringShouldContain "Budget"
            }
        }
