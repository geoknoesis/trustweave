...
class IdiomaticApiTest {
    ...
    fun `test resolver builder DSL`() {
        val resolver =
            universalResolver("https://dev.uniresolver.io") {
                timeout = 60
                apiKey = "test-key"
                retry {
                    maxRetries = 5
    ...
    @Test
    fun `test DidResolutionResult extensions`() =
        runBlocking<Unit> {
            val success =
    ...
    @Test
    fun `test DidResolutionResult failure extensions`() =
        runBlocking<Unit> {
            val failure =
    ...
    @Test
    fun `test Did extension functions`() =
        runBlocking<Unit> {
            val did = Did("did:test:123")
            val resolver =
                object : DidResolver {
                    override suspend fun resolve(did: Did) =
                        DidResolutionResult.Success(
                            DidDocument(id = did),
                        )
                }
