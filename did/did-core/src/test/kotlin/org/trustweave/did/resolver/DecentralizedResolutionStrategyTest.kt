...
class DecentralizedResolutionStrategyTest {
    ...
    @Test
    fun `a first-stage Deactivated verdict short-circuits and is returned unchanged`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `a first-stage Failure still falls through to the next stage`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `a method-specific Deactivated verdict short-circuits before the universal resolver`() =
        runBlocking<Unit> {
            val deactivated = DidResolutionResult.Deactivated(did)
    ...
    @Test
    fun `a stale local Success still falls through to the next stage`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `DecentralizedResolutionStrategy forwards options to whichever stage answers`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `ResolutionFallbackStrategy forwards options to every resolver it tries`() =
        runBlocking<Unit> {
            val options = ResolutionOptions(accept = "application/did+ld+json")
