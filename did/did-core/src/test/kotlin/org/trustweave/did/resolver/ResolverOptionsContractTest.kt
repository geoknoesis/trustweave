...
class ResolverOptionsContractTest {
    ...
    private val method =
        DidMethodResolver { requested ->
            DidResolutionResult.Success(document = DidDocument(id = requested))
        }

    @Test
    fun `empty options delegate to the single-argument form`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `method-independent options still delegate`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `an unsupported versionId yields FEATURE_NOT_SUPPORTED`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `an unsupported versionTime yields FEATURE_NOT_SUPPORTED`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `an unsupported noCache yields FEATURE_NOT_SUPPORTED`() =
        runBlocking<Unit> {
            assertEquals(
    ...
    @Test
    fun `invalid options yield INVALID_OPTIONS ahead of feature support`() =
        runBlocking<Unit> {
            val options =
                ResolutionOptions(
                    versionId = "3",
    ...
    @Test
    fun `a DidResolver gets the same default behaviour`() =
        runBlocking<Unit> {
