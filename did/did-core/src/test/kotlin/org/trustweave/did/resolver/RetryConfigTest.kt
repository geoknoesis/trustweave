...
class RetryConfigTest {
    ...
    @Test
    fun `test executeWithRetry succeeds on first attempt`() =
        runBlocking<Unit> {
            val config = RetryConfig.default()
            var attempts = 0
    ...
    @Test
    fun `test executeWithRetry retries on retryable exception`() =
        runBlocking<Unit> {
            val config =
                RetryConfig(
                    maxRetries = 2,
    ...
    @Test
    fun `test executeWithRetry fails after max retries`() =
        runBlocking<Unit> {
            val config =
                RetryConfig(
                    maxRetries = 2,
    ...
    @Test
    fun `test executeWithRetry does not retry non-retryable exceptions`() =
        runBlocking<Unit> {
            val config = RetryConfig.default()
            var attempts = 0
    ...
    @Test
    fun `test custom retryable exceptions`() =
        runBlocking<Unit> {
            val config =
                RetryConfig(
                    maxRetries = 1,
