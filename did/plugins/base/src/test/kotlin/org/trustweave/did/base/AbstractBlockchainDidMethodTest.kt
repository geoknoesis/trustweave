...
class AbstractBlockchainDidMethodTest {
    ...
    private class TestBlockchainDidMethod(
        ...
        override suspend fun resolveDid(did: Did): DidResolutionResult = resolveFromBlockchain(did.value)
    ...
    private class FailingReadAnchorClient(
        ...
        override suspend fun writePayload(
            payload: JsonElement,
            mediaType: String,
        ): AnchorResult = delegate.writePayload(payload, mediaType)

        override suspend fun readPayload(ref: AnchorRef): AnchorResult = throw IllegalStateException("chain unavailable")
    }
    ...
    private fun deactivatedCopy(document: DidDocument): DidDocument =
        document.copy(
    ...
    @Test
    fun `stored-document fallback reports deactivated=false for an active did`() =
        runBlocking {
            val method =
    ...
    @Test
    fun `stored-document fallback surfaces Deactivated after deactivation`() =
        runBlocking {
            val method =
                TestBlockchainDidMethod(
                    InMemoryKeyManagementService(),
    ...
    @Test
    fun `exception fallback surfaces Deactivated after deactivation`() =
        runBlocking {
    ...
    @Test
    fun `a deactivated blockchain DID resolves to Deactivated with no document`() =
        runBlocking {
