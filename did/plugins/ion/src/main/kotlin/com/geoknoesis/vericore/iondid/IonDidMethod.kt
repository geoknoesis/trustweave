package com.geoknoesis.vericore.iondid
...
class IonDidMethod(
        ...
        try {
            ...
            val createOperation = sidetreeClient.createCreateOperation(
                publicKeyJwk = keyHandle.publicKeyJwk
                    ?: throw VeriCoreException("Public key JWK is required for did:ion")
            )
            ...
            val longFormDid = operationResponse.longFormDid
                ?: throw VeriCoreException("Failed to create did:ion: no DID in response")
    ...
    override suspend fun resolveDid(did: String): DidResolutionResult = withContext(Dispatchers.IO) {
        try {
            ...
            // Convert ION document to VeriCore format
    ...
    private fun buildIonDocument(
        did: String,
        keyHandle: com.geoknoesis.vericore.kms.KeyHandle,
    ...
    /**
     * Converts ION document JSON to VeriCore DidDocument.
     */
