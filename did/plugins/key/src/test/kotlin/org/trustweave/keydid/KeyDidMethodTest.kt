...
class KeyDidMethodTest {
    ...
    /**
     * Builds a did:key whose multicodec prefix claims [prefix] but whose key body is
     * [bodySize] bytes, so the declared algorithm and the actual key material disagree.
     */
    private fun didKeyWithBody(
        prefix: ByteArray,
        bodySize: Int,
    ): Did {
        val prefixed = prefix + ByteArray(bodySize) { 0x11 }
        return Did("did:key:z" + prefixed.encodeBase58())
    }

    @Test
    fun `resolving an Ed25519 did-key with a wrong-length body fails`() =
        runBlocking {
            // 0xed 0x01 is the ed25519-pub multicodec; Ed25519 public keys are exactly 32 bytes.
            // A short body must not be accepted and re-published as this DID's verification key.
            val did = didKeyWithBody(byteArrayOf(0xed.toByte(), 0x01), bodySize = 8)

            val result = method.resolveDid(did)

            assertTrue(
                result !is DidResolutionResult.Success,
                "A did:key whose Ed25519 key body is not 32 bytes must not resolve, got: $result",
            )
        }

    @Test
    fun `resolving an X25519 did-key with a wrong-length body fails`() =
        runBlocking {
            // 0xec 0x01 is the x25519-pub multicodec; X25519 public keys are exactly 32 bytes.
            val did = didKeyWithBody(byteArrayOf(0xec.toByte(), 0x01), bodySize = 100)

            val result = method.resolveDid(did)

            assertTrue(
                result !is DidResolutionResult.Success,
                "A did:key whose X25519 key body is not 32 bytes must not resolve, got: $result",
            )
        }
    ...
    @Test
    fun `test create DID with Ed25519`() =
        runBlocking {
            val document =
                method.createDid(
                    didCreationOptions {
    ...
    @Test
    fun `test resolve DID after creation`() =
        runBlocking {
            val document =
                method.createDid(
                    didCreationOptions {
    ...
    @Test
    fun `test DID format includes multibase prefix`() =
        runBlocking {
            val document =
                method.createDid(
                    didCreationOptions {
    ...
    @Test
    fun `test created secp256k1 did encodes a compressed point`() =
        runBlocking {
            val document =
                method.createDid(
                    didCreationOptions {
    ...
    @Test
    fun `test resolve compressed secp256k1 did from did-key test vectors`() =
        runBlocking {
    ...
    @Test
    fun `test resolve compressed P-256 did from did-key test vectors`() =
        runBlocking {
    ...
    @Test
    fun `test legacy uncompressed secp256k1 did is still accepted on parse`() =
        runBlocking {
    ...
    @Test
    fun `test resolve x25519 did produces key agreement only document`() =
        runBlocking {
    ...
    @Test
    fun `test created secp256k1 did round-trips through resolution`() =
        runBlocking {
            val document =
                method.createDid(
                    didCreationOptions {
