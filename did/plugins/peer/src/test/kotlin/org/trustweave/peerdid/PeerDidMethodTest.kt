...
class PeerDidMethodTest {
    ...
    @Test
    fun `numalgo2 service segment is base64url not multibase`() =
        runBlocking<Unit> {
            val document =
                newMethod().createDid(
                    didCreationOptions {
    ...
    @Test
    fun `numalgo2 service round-trips through a fresh resolver instance`() =
        runBlocking<Unit> {
            val document =
                newMethod().createDid(
                    didCreationOptions {
    ...
    @Test
    fun `numalgo2 cross-vendor fixture from did-peer spec is parsed`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `numalgo2 all six purpose codes are decoded to the correct relationships`() =
        runBlocking<Unit> {
            // Synthetic keys: distinct Ed25519 keys for V/A/I/D, an X25519 key for E.
            fun ed25519Mb(seed: Int) = "z" + (byteArrayOf(0xed.toByte(), 0x01) + ByteArray(32) { (it + seed).toByte() }).encodeBase58()
            val x25519Mb =
    ...
    @Test
    fun `numalgo2 A segment maps to assertionMethod not authentication`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `numalgo2 createDid encodes I and D purpose segments and round-trips`() =
        runBlocking<Unit> {
            val document =
                newMethod().createDid(
                    didCreationOptions {
    ...
    @Test
    fun `numalgo2 stored document is identical to what a third party resolves`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `numalgo2 KEY_AGREEMENT-only creation is consistent between stored and resolved`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `numalgo2 stored service id matches the spec parser convention`() =
        runBlocking<Unit> {
            val document =
                newMethod().createDid(
                    didCreationOptions {
    ...
    @Test
    fun `legacy multibase service segment is still accepted on parse`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `numalgo1 numeric basis is a multihash of the genesis document`() =
        runBlocking<Unit> {
            val document =
                newMethod(PeerDidConfig.numalgo1()).createDid(
                    didCreationOptions {
                        algorithm = KeyAlgorithm.ED25519
                    },
                )
    ...
    @Test
    fun `numalgo0 did round-trips through a fresh resolver instance`() =
        runBlocking<Unit> {
            val document =
                newMethod(PeerDidConfig.numalgo0()).createDid(
                    didCreationOptions {
                        algorithm = KeyAlgorithm.ED25519
                    },
                )
