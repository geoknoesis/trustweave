...
An editor signs an article identifier, revision and author attribution.
...
```sh
./gradlew :distribution:examples:runNewsIndustry
```
...
[NewsIndustryExample.kt](src/main/kotlin/NewsIndustryExample.kt) defines the credential type, claims and an adversarial change.
...
Expected output: `news-industry: original verified; JSON round-trip verified; tampering rejected`.
The issuer and holder DIDs are generated locally on each run. No credentials or keys persist.

## Scope

Authenticity of the signed claims does not establish factual accuracy of the article. This demonstrates integrity and issuer-key possession with synthetic data,
