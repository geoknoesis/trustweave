...
fun main() =
    runBlocking {
        ...
        val chain =
            org.trustweave.trust.dsl.did
                .DelegationBuilder(trustWeave)
                .verifyChain(listOf(ceoDid.value, hrDirectorDid.value, hrManagerDid.value))
        check(chain.valid) { "Delegation chain failed: ${chain.errors}" }
        val unrelated =
            org.trustweave.trust.dsl.did
                .DelegationBuilder(trustWeave)
                .verifyChain(listOf(ceoDid.value, employeeDid.value))
        check(!unrelated.valid) { "An undelegated employee was accepted" }
        println("Multi-hop delegation verified; undelegated employee rejected")
        println()
