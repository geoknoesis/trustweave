...
fun main() = runBlocking {
    ...
    val trustweave = TrustWeave.build {
        ...
        keys {
            custom(kmsRef)
            signer { data, keyId ->
                when (val signResult = kmsRef.sign(org.trustweave.core.identifiers.KeyId(keyId), data)) {
    ...
    if (proof != null) {
        ...
        val proofPurpose = when (proof) {
            is org.trustweave.credential.model.vc.CredentialProof.LinkedDataProof -> proof.proofPurpose
            else -> null
        }
