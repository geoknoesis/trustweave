...
fun main() =
    runBlocking {
        ...
        val universityDidResolution =
            trustWeave.configuration.didRegistry.resolve(universityDid.value)
                ?: throw IllegalStateException("Failed to resolve university DID")
        val universityDidDoc =
            when (universityDidResolution) {
        ...
        val universityVerificationMethod =
            universityDidDoc.verificationMethod.firstOrNull()
                ?: throw IllegalStateException("No verification method found in university DID document")
        val universityKeyId =
            universityVerificationMethod.extractKeyId()
        ...
        checkNotNull(awsCertStored)
        val schemaResult = trustWeave.schema("https://example.com/schemas/certification").validate(awsCertStored)
        check(schemaResult.valid) { "Certification schema validation failed: ${schemaResult.errors}" }
        println("AWS cert schema validation passed")
        ...
        val credentialServiceForPresentation =
            credentialService(
        ...
        val jobApplicationCredentials: List<VerifiableCredential> =
            listOf(
                masterId.value,
                job1Id,
                job2Id,
        ...
        val jobApplicationPresentation =
            credentialServiceForPresentation.createPresentation(
