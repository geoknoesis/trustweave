...
import com.trustweave.examples.createTestProofRegistry
import com.trustweave.credential.verifier.CredentialVerifier
import com.trustweave.credential.wallet.CredentialQueryBuilder
import com.trustweave.credential.did.CredentialDidResolver
import com.trustweave.examples.canResolveDid
import com.trustweave.examples.createTestDidRegistry
import com.trustweave.examples.createTestProofRegistry
...
class AcademicCredentialsExampleTest {
    ...
    fun `test DID creation`() = runBlocking {
        ...
        assertTrue(didRegistry.canResolveDid(universityDid.id))
        assertTrue(didRegistry.canResolveDid(studentDid.id))
    }
    ...
    fun `test credential verification`() = runBlocking {
        ...
        // Get the key ID from the DID document's verification method
        val verificationMethodId = universityDid.verificationMethod.first().id
        val keyId = verificationMethodId.substringAfter("#")
        val proofGenerator = Ed25519ProofGenerator(
            signer = { data, _ -> universityKms.sign(keyId, data) },
...
fun createDegreeCredential(
    ...
    val credentialSubject = buildJsonObject {
        put("id", studentDid)
        put("type", "DegreeSubject")
        put("degreeName", degreeName)
        put("universityName", universityName)
        put("graduationDate", graduationDate)
        put("gpa", gpa)
    }

    return VerifiableCredential(
        id = "urn:example:degree:${System.currentTimeMillis()}",
