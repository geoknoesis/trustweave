...
class ProfessionalIdentityExampleTest {
    @Test
    fun `test main function executes successfully`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `test storing multiple credential types`() =
        runBlocking<Unit> {
            // Setup
    ...
    @Test
    fun `test credential organization with collections`() =
        runBlocking<Unit> {
            // Setup
            val kms = InMemoryKeyManagementService()
            val didMethod = DidKeyMockMethod(kms)
    ...
    @Test
    fun `test credential querying by type`() =
        runBlocking<Unit> {
            // Setup
            val kms = InMemoryKeyManagementService()
            val didMethod = DidKeyMockMethod(kms)
    ...
    @Test
    fun `test selective disclosure presentation`() =
        runBlocking<Unit> {
            // Setup
            val kms = InMemoryKeyManagementService()
            val didMethod = DidKeyMockMethod(kms)
    ...
    @Test
    fun `test full presentation creation`() =
        runBlocking<Unit> {
            // Setup
            val kms = InMemoryKeyManagementService()
            val didMethod = DidKeyMockMethod(kms)
            ...
            val presentation =
                wallet.createPresentation(
                    ...
                    holderDid = professionalDid.value,
                    options =
                        org.trustweave.credential.proof
                            .ProofOptions(),
                )
    ...
    @Test
    fun `test querying active certifications`() =
        runBlocking<Unit> {
            // Setup
            val kms = InMemoryKeyManagementService()
            val didMethod = DidKeyMockMethod(kms)
