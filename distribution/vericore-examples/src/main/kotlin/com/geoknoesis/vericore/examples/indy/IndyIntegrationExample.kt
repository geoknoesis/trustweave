...
fun main() = runBlocking {
    ...
    try {
        val issuerResolution = vericore.dids.resolve(issuerDid.id)
    ...
    try {
        val holderResolution = vericore.dids.resolve(holderDid.id)
    ...
    val credential = try {
        ...
        println("  Available methods: ${error.availableMethods.joinToString(", ")}")
        return@runBlocking
    } catch (error: VeriCoreError.CredentialInvalid) {
    ...
    val verification = try {
        vericore.credentials.verify(credential)
    } catch (error: VeriCoreError.CredentialInvalid) {
        println("✗ Credential validation failed: ${error.reason}")
        println("  Field: ${error.field}")
        return@runBlocking
    ...
    val wallet = try {
        vericore.wallets.create(holderDid = holderDid.id)
    } catch (error: VeriCoreError.WalletCreationFailed) {
    ...
    val readVerification = try {
        vericore.credentials.verify(readCredential)
    } catch (error: Throwable) {
        println("✗ Verification failed: ${error.message}")
        return@runBlocking
    }
    ...
    for (i in 1..2) {
        try {
            val additionalCredential = vericore.credentials.issue(
                issuer = issuerDid.id,
                subject = buildJsonObject {
                    put("id", holderDid.id)
    ...
    try {
        vericore.dids.resolve("did:unknown:test")
