...
- **Wallet DSL** – expose DID creation to end-users through `wallet { did { ... } }` (powered by `DidManagement`). See Wallets.
...
- **Error handling** – `createDid` / `resolveDid` return sealed results; `updateDid` / `rotateKey` return documents or throw domain exceptions. See [Error handling](../api-reference/advanced/error-handling.md).
- **Input validation** – TrustWeave automatically validates DID format and method registration before operations.
- **DSL-based API** – Access DID operations through `trustWeave.createDid { }`, `trustWeave.resolveDid()`, `trustWeave.updateDid { }`, and `trustWeave.rotateKey { }`.

## Standards: DID Core 1.1

W3C **Decentralized Identifiers v1.1** layers on **Controlled Identifiers** and moves much of **resolution** into a separate spec. TrustWeave today targets **DID Core 1.0** semantics in docs and defaults; for a **comparison of the specs** and **documented implementation gaps** (embedded verification methods in relationship arrays, `alsoKnownAs` URLs, DID Resolution v0.3, test suite, etc.), see **[DID Core 1.0 vs 1.1 and TrustWeave gaps](../api-reference/did-core-1-1-compliance-and-gaps.md)**.

## Related How-To Guides

- **[Create and Manage DIDs](../how-to/create-dids.md)** - Step-by-step guide for creating, resolving, updating, and deactivating DIDs
...
- [DID Integration Guides](../how-to/integrations/README.md#did-method-integrations) - Implementation guides for all DID methods
