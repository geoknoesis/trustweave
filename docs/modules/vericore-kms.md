...
The `vericore-kms` module provides the key management service abstraction used throughout VeriCore. This module defines the interface for key generation, signing, and management operations.
...
The `vericore-kms` module provides:
...
**Outcome:** Allows VeriCore to work with any key management backend (HSM, cloud KMS, in-memory, etc.) through a unified interface.
...
VeriCore provides several KMS implementations:

- **In-Memory** (`com.geoknoesis.vericore:vericore-testkit`) – For testing and development. See [TestKit Documentation](vericore-testkit.md).
- **AWS KMS** (`com.geoknoesis.vericore.kms:aws`) – AWS Key Management Service integration. See [AWS KMS Integration Guide](../integrations/aws-kms.md).
...
- **HashiCorp Vault** (`com.geoknoesis.vericore.kms:hashicorp`) – HashiCorp Vault Transit engine integration. See [HashiCorp Vault KMS Integration Guide](../integrations/hashicorp-vault-kms.md).
- **walt.id** (`com.geoknoesis.vericore.kms:waltid`) – walt.id KMS integration. See [walt.id Integration Guide](../integrations/waltid.md).
...
- Upstream modules (DID methods, credential services) depend on `vericore-kms` for key operations
...
- See [`vericore-testkit`](vericore-testkit.md) for in-memory implementations for testing
