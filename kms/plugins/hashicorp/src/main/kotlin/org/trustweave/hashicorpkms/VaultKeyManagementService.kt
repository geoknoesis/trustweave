    ...
    override suspend fun generateKey(
        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                val publicKeyPem =
                    VaultPublicKeyResponse.extract(keyInfo)
                ...
                val context =
                    KmsErrorHandler.createErrorContext(
                        algorithm = algorithm,
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                if (httpCode == 404) {
                    ...
                    logger.error(
                        "Failed to get public key from Vault Transit",
                        mapOf(
    ...
    override suspend fun sign(
        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                val signature =
                    signResponse.data["signature"] as? String
                        ?: return@withContext SignResult.Failure.Error(
                            keyId = keyId,
                ...
                if (httpCode == 404) {
                    ...
                    logger.error(
                        "Failed to sign data in Vault Transit",
                        mapOf(
                            "keyId" to keyId.value,
                            "httpCode" to (httpCode ?: "unknown"),
                            "vaultAddress" to config.address,
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                if (httpCode == 404) {
                    ...
                    logger.error(
                        "Failed to delete key from Vault Transit",
                        mapOf(
                            "keyId" to keyId.value,
                            "httpCode" to (httpCode ?: "unknown"),
                            "vaultAddress" to config.address,
                    ...
                    DeleteKeyResult.Failure.Error(
                        keyId = keyId,
                        reason =
                            "Failed to delete key: ${e.message ?: "Unknown error"}. " +
