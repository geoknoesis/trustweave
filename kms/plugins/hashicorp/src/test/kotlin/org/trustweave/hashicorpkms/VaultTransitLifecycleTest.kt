...
class VaultTransitLifecycleTest {
    ...
    ) : GenericContainer<VaultContainer>(image)

    @Test
    fun `create retrieve sign reject and reopen against real Transit`() =
        runBlocking {
            val container =
                VaultContainer(
            ...
            try {
                ...
                assertTrue(
                    Regex("transit/keys/ed25519-[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}")
                        .matches(key.id.value),
                    "Generated names must retain the complete UUID",
                )
                ...
                verifier.initVerify(javaKey)
                verifier.update("altered".toByteArray())
