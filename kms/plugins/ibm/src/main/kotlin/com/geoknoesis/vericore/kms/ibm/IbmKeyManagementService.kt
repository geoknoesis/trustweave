...
class IbmKeyManagementService(
    ...
    override suspend fun generateKey(
        ...
        try {
            ...
            val resources = jsonResponse["resources"]?.jsonArray ?: throw VeriCoreException("Invalid response from IBM Key Protect")
            val keyResource = resources.firstOrNull()?.jsonObject ?: throw VeriCoreException("No key in response")
    ...
    override suspend fun getPublicKey(keyId: String): KeyHandle = withContext(Dispatchers.IO) {
        try {
            ...
            val keyType = keyResource["type"]?.jsonPrimitive?.content
                ?: throw VeriCoreException("Key type not found in response")
            val algorithm = AlgorithmMapping.fromIbmKeyType(keyType)
                ?: throw VeriCoreException("Unknown key type: $keyType")
            ...
            val publicKeyResources = publicKeyJson["resources"]?.jsonArray ?: throw VeriCoreException("Invalid public key response")
            val publicKeyResource = publicKeyResources.firstOrNull()?.jsonObject ?: throw VeriCoreException("No public key in response")
    ...
    override suspend fun sign(
        ...
        try {
            ...
            val jsonResponse = kotlinx.serialization.json.Json.parseToJsonElement(responseBody ?: "{}").jsonObject
            val resources = jsonResponse["resources"]?.jsonArray ?: throw VeriCoreException("Invalid response from IBM Key Protect")
            val signResource = resources.firstOrNull()?.jsonObject ?: throw VeriCoreException("No signature in response")
