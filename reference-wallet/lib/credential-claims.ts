 ...
 * own detail view.
 *
 * The wallet stores the full credential (VC-JWT or SD-JWT VC). The claim *values*
 * live inside that string; this module decodes them so the detail panel can show
 * the actual data — not just the claim names.
 *
 ...
 * omitted (the portrait is shown separately as a photo). Any JWE-protected
 * disclosure value is decrypted with the holder's key.
 */
import { decodeSdJwtVc } from './sdjwt'
import { b64uDecode, b64uDecodeString } from './crypto'
import { decryptClaimJwe, isClaimJwePayload } from './claim-jwe'
import { humanClaimName } from './credential-display'
import { loadHolder, type StoredCredential } from './storage'
...
/** JWT / SD-JWT envelope and subject-id fields that are not credential data. */
const ENVELOPE_KEYS = new Set([
  'iss', 'sub', 'aud', 'iat', 'exp', 'nbf', 'jti', 'cnf',
  'vct', 'vc', 'type', '@context', 'id', '_sd', '_sd_alg', 'status',
])
...
function flatten(rawName: string, value: unknown, depth: number, shareable: boolean, keyPath: string, out: ClaimRow[]): void {
  ...
  if (Array.isArray(value)) {
    ...
    // Array of primitives → one readable line ("admin, user").
    if (value.every(isPrimitive)) {
      out.push({ key: keyPath, label, value: value.map(String).join(', '), depth, shareable })
      return
    }
    // Array of objects → header + an indented group per item.
  ...
  // Object → header + indented sub-rows.
 ...
 * Async because SD-JWT JWE claim values are decrypted with the holder's key.
 * Never throws — returns `[]` on a malformed credential.
 */
export async function extractClaimsForDisplay(cred: StoredCredential): Promise<ClaimRow[]> {
  ...
  let holderPriv: Uint8Array | null = null
  const holder = loadHolder()
  if (holder) {
    try {
      holderPriv = b64uDecode(holder.privateKey)
    } catch {
      holderPriv = null
    }
  }
  ...
  for (const [name, value] of Object.entries(raw)) {
    ...
    if (isClaimJwePayload(value)) {
      if (!holderPriv) continue // cannot decrypt without the holder key
      try {
        resolved = await decryptClaimJwe(value, holderPriv)
      } catch {
        resolved = '••• (could not decrypt)'
      }
    }

    flatten(name, resolved, 0, shareable.has(name), name, rows)
  }

  // Stable top-level order; nested rows keep their natural insertion order.
  return rows
}
