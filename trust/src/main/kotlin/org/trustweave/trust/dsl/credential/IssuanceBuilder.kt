...
class IssuanceBuilder(
     ...
     */
    suspend fun build(): IssuanceResult =
        withContext(ioDispatcher) {
            ...
            val cred =
                credential ?: return@withContext IssuanceResult.Failure.InvalidRequest(
                    field = "credential",
                    reason = "Credential is required. Use credential { ... } to build the credential.",
                )
            val resolvedIssuerDid =
                issuerDid ?: return@withContext IssuanceResult.Failure.InvalidRequest(
                ...
                if (issuerKeyId != null) {
                    ...
                    if (didResolver == null) {
                        return@withContext IssuanceResult.Failure.InvalidRequest(
                            field = "issuerKeyId",
                            reason =
                                "Issuer key ID is required and no DID resolver is configured for auto-extraction. " +
            ...
            val resolvedKeyIdNonNull =
                resolvedKeyId ?: return@withContext IssuanceResult.Failure.InvalidRequest(
                    field = "issuerKeyId",
                    reason = "Could not determine issuer key ID",
                )
            val normalizedKeyId =
                if (resolvedKeyIdNonNull.contains("#")) {
            ...
            if (autoRevocation && credentialToIssue.credentialStatus == null) {
                if (revocationManager != null) {
                    ...
                    val statusListId =
                        try {
                            ...
                            return@withContext IssuanceResult.Failure.AdapterError(
                                ...
                                reason =
                                    "Failed to find or create status list for revocation. " +
                    ...
                    val credentialId =
                        credentialToIssue.id?.value ?: java.util.UUID
                            .randomUUID()
                            .toString()
                    if (credentialToIssue.id == null) {
                        credentialToIssue =
                            credentialToIssue.copy(
                                id =
                    ...
                    return@withContext IssuanceResult.Failure.AdapterError(
                        format = proofSuite ?: defaultProofSuite,
                        reason =
                            "Revocation requested but revocation manager is not configured. " +
