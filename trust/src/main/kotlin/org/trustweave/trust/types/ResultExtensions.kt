...
fun VerificationResult.getOrThrow(): VerifiableCredential =
    when (this) {
        ...
        is VerificationResult.Invalid.InvalidIssuer -> {
            val suggestion =
                " Ensure the issuer DID is valid and the DID method is properly configured. " +
        ...
        is VerificationResult.Invalid.UntrustedIssuer -> {
            val suggestion =
                " To trust this issuer, add it to the trust registry: " +
        ...
        is VerificationResult.Invalid.SchemaValidationFailed -> {
            val suggestion =
                " Ensure the credential schema is valid and matches the credential structure. " +
 ...
 * ```
 */
fun DidCreationResult.getOrThrowDid(): Did = getOrThrow().first
 ...
 * ```
 */
fun DidCreationResult.getOrThrowDocument(): DidDocument = getOrThrow().second
        ...
        is WalletCreationResult.Failure.StorageFailed -> {
            throw IllegalStateException(
                "Wallet storage failed: $reason. " +
                    "This may indicate an issue with the wallet storage backend or configuration.",
                cause,
            )
        }
        is WalletCreationResult.Failure.Other -> {
            throw IllegalStateException(
                "Wallet creation failed: $reason",
                cause,
            )
        }
    }
