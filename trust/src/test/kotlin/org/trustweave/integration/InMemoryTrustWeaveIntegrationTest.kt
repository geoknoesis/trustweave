...
import org.trustweave.trust.from
import org.trustweave.trust.types.errors
import org.trustweave.trust.types.getOrThrow
import org.trustweave.trust.types.getOrThrowDid
import org.trustweave.trust.types.issuerValid
import org.trustweave.trust.types.proofValid
import org.trustweave.trust.types.revoked
import org.trustweave.trust.types.trustRegistryValid
import org.trustweave.trust.types.valid
import org.trustweave.trust.types.warnings
 ...
 */
class InMemoryTrustWeaveIntegrationTest {
    /**
    ...
    private suspend fun createTrustWeaveWithCredentialService(
        ...
        val sharedRevocationManager =
            org.trustweave.credential.revocation.RevocationManagers
                .default()
        ...
        val patchedConfig =
            finalTrustWeave.configuration.copy(
                credentialService = credentialService,
                revocationManager = sharedRevocationManager,
            )
        return TrustWeave.from(patchedConfig)
    }

    @Test
    fun `test complete in-memory workflow template`() =
        runBlocking<Unit> {
    ...
    fun `test with external services template`() =
        runBlocking<Unit> {
     ...
     */
    @Test
    fun `test credential revocation workflow template`() =
        runBlocking<Unit> {
            val kms = InMemoryKeyManagementService()

            val trustWeave =
                createTrustWeaveWithCredentialService(kms) {
            ...
            assertTrue(initialResult is VerificationResult.Valid, "Credential should not be revoked initially")
            ...
            assertTrue(
                revokedResult is org.trustweave.credential.results.VerificationResult.Invalid.Revoked,
     ...
     */
    @Test
    fun `test wallet storage workflow template`() =
        runBlocking<Unit> {
            val kms = InMemoryKeyManagementService()
     ...
     */
    @Test
    fun `test verifiable presentation workflow template`() =
        runBlocking<Unit> {
            val kms = InMemoryKeyManagementService()
            ...
            val holderDidResolution =
                trustWeave.configuration.didRegistry.resolve(holderDid.value)
                    ?: throw IllegalStateException("Failed to resolve holder DID")
            val holderDidDoc =
                when (holderDidResolution) {
            ...
            // Create verifiable presentation using the DSL (result API + unwrap in tests)
            val presentation =
                trustWeave
                    .presentationResult {
     ...
     */
    @Test
    fun `test DID update workflow template`() =
        runBlocking<Unit> {
            val kms = InMemoryKeyManagementService()
            ...
            val newKey =
                when (val result = kms.generateKey("Ed25519")) {
     ...
     */
    @Test
    fun `test blockchain anchoring workflow template`() =
        runBlocking<Unit> {
            val kms = InMemoryKeyManagementService()
     ...
     */
    @Test
    fun `test smart contract workflow template`() =
        runBlocking<Unit> {
            val kms = InMemoryKeyManagementService()
