...
class WebOfTrustIntegrationTest {
    ...
    @Test
    fun `test complete trust registry workflow`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `test delegation chain with credential issuance`() =
        runBlocking<Unit> {
            val trustWeave =
                TrustWeave.build {
    ...
    @Test
    fun `test proof purpose validation in credential verification`() =
        runBlocking<Unit> {
            val trustWeave =
                TrustWeave.build {
                    // DID methods auto-discovered via SPI
