...
class TimeoutFailureMappingTest {
    ...
    @Test
    fun `createDid exceeding timeout returns Failure instead of throwing`() =
        runBlocking<Unit> {
            installSlowKeyMethod()

            val result =
                trustWeave.createDid(timeout = shortTimeout) {
                    method("key")
                    algorithm("Ed25519")
                }
    ...
    @Test
    fun `resolveDid exceeding timeout returns Failure instead of throwing`() =
        runBlocking<Unit> {
            val did =
                trustWeave
                    .createDid {
    ...
    @Test
    fun `updateDid exceeding timeout returns Failure instead of throwing`() =
        runBlocking<Unit> {
            val did =
                trustWeave
                    .createDid {
            ...
            val result =
                trustWeave.updateDid(timeout = shortTimeout) {
                    did(did.value)
                    removeKey("$did#none")
                }
    ...
    @Test
    fun `rotateKey exceeding timeout returns Failure instead of throwing`() =
        runBlocking<Unit> {
            val did =
                trustWeave
                    .createDid {
            ...
            val result =
                trustWeave.rotateKey(timeout = shortTimeout) {
                    did(did.value)
                    algorithm("Ed25519")
                }
    ...
    @Test
    fun `delegate exceeding timeout returns invalid result instead of throwing`() =
        runBlocking<Unit> {
            val delegator =
                trustWeave
                    .createDid {
            ...
            val result =
                trustWeave.delegate(timeout = shortTimeout) {
    ...
    @Test
    fun `issue exceeding timeout returns Failure instead of throwing`() =
        runBlocking<Unit> {
            ...
            val result =
                issuingTrustWeave.issue(timeout = shortTimeout) {
                    credential {
                        type("TestCredential")
                        issuer(issuerDid)
                        subject {
    ...
    @Test
    fun `verify exceeding timeout returns Invalid instead of throwing`() =
        runBlocking<Unit> {
    ...
    fun `revoke exceeding timeout throws OperationTimedOut instead of leaking TimeoutCancellationException`() =
        runBlocking<Unit> {
            ...
            val exception =
                assertFailsWith<TrustWeaveException.OperationTimedOut> {
    ...
    @Test
    fun `revoke cancellation still propagates and is not mapped to OperationTimedOut`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `real cancellation still propagates and is not mapped to a Failure`() =
        runBlocking<Unit> {
            val did =
                trustWeave
                    .createDid {
