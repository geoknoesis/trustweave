...
class TrustWeaveProductionApiContractTest {
    @Test
    fun `issue returns AdapterNotReady when credential service is null`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `presentationResult returns AdapterNotReady when credential service is null`() =
        runBlocking<Unit> {
            val tw = trustWeaveWithoutCredentialService()
    ...
    @Test
    fun presentationResultGetOrThrowThrowsInvalidStateWhenAdapterNotReady() =
        runBlocking<Unit> {
            val tw = trustWeaveWithoutCredentialService()
            val ex =
    ...
    @Test
    fun `presentationResult returns InvalidRequest when holder missing`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `issueBatch emits AdapterNotReady for each request when service missing`() =
        runBlocking<Unit> {
            val tw = trustWeaveWithoutCredentialService()
            val results =
                tw
    ...
    @Test
    fun `verifyBatch emits AdapterNotReady for each credential when service missing`() =
        runBlocking<Unit> {
            val tw = trustWeaveWithoutCredentialService()
            val c =
            ...
            val minimal =
                org.trustweave.credential.model.vc.VerifiableCredential(
                    ...
                    issuer =
                        org.trustweave.credential.model.vc.Issuer.IriIssuer(
                            org.trustweave.core.identifiers
                                .Iri("did:key:batch-a"),
                        ),
                    issuanceDate = c,
                    credentialSubject =
                        org.trustweave.credential.model.vc.CredentialSubject.fromDid(
                            org.trustweave.did.identifiers
                                .Did("did:key:batch-b"),
                        ),
                )
