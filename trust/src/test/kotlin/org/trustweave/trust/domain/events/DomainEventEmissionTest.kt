...
private class StubAccount(
    ...
    override suspend fun estimateFee(op: OperationDescriptor): TokenAmount = TokenAmount(chainId, AssetRef.Native, fee)
...
class DomainEventEmissionTest {
    ...
    private fun result(fee: BigInteger): AnchorResult =
        AnchorResult(
    ...
    @Test
    fun `reserve then settle emits Reserved then Settled`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `reserve then cancel emits Reserved then Cancelled`() =
        runBlocking<Unit> {
            val sink = RecordingSink()
            val account = StubAccount(BigInteger.valueOf(1_000_000), BigInteger.valueOf(10_000))
            val treasury =
    ...
    @Test
    fun `settle with success=false emits Failed`() =
        runBlocking<Unit> {
            val sink = RecordingSink()
            val account = StubAccount(BigInteger.valueOf(1_000_000), BigInteger.valueOf(10_000))
            val treasury =
