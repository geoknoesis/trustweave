...
private class StubChainAccount(
    ...
    override suspend fun estimateFee(op: OperationDescriptor): TokenAmount = TokenAmount(chainId, AssetRef.Native, feeEstimate)

    override suspend fun sign(tx: UnsignedTx): SignedTx = SignedTx(tx.chainId, tx.bytes)
}

class InMemoryDomainTreasuryTest {
    private fun anchorResult(feeAmount: BigInteger): AnchorResult =
        AnchorResult(
    ...
    private fun ctx(
        maxFee: BigInteger? = null,
        strategy: FeeStrategy = FeeStrategy.DomainPays,
    ) = PaymentContext(
    ...
    @Test
    fun `reserve and settle records actual fee in ledger`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `reserve rejects when balance below estimate plus safety margin`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `cancel releases the lock and marks entry cancelled`() =
        runBlocking<Unit> {
            val account = StubChainAccount(initialBalance = BigInteger.valueOf(1_000_000), feeEstimate = BigInteger.valueOf(10_000))
            val treasury =
                InMemoryDomainTreasury(
    ...
    @Test
    fun `caller maxFee below estimate raises CallerCapExceeded before submission`() =
        runBlocking<Unit> {
            val account = StubChainAccount(initialBalance = BigInteger.valueOf(1_000_000), feeEstimate = BigInteger.valueOf(10_000))
            val treasury =
                InMemoryDomainTreasury(
    ...
    @Test
    fun `per-operation cap rejects oversized estimate`() =
        runBlocking<Unit> {
            val account = StubChainAccount(initialBalance = BigInteger.valueOf(1_000_000), feeEstimate = BigInteger.valueOf(10_000))
            val treasury =
                InMemoryDomainTreasury(
    ...
    @Test
    fun `strategy not in allowedStrategies raises StrategyNotAllowed`() =
        runBlocking<Unit> {
            val account = StubChainAccount(initialBalance = BigInteger.valueOf(1_000_000), feeEstimate = BigInteger.valueOf(10_000))
            val treasury =
                InMemoryDomainTreasury(
    ...
    @Test
    fun `concurrent reservations cannot oversubscribe balance`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `sponsor allow-list rejects unauthorised sponsor`() =
        runBlocking<Unit> {
            val account = StubChainAccount(initialBalance = BigInteger.valueOf(1_000_000), feeEstimate = BigInteger.valueOf(10_000))
            val treasury =
                InMemoryDomainTreasury(
    ...
    @Test
    fun `per-window cap aggregates prior settled fees`() =
        runBlocking<Unit> {
            val account = StubChainAccount(initialBalance = BigInteger.valueOf(1_000_000), feeEstimate = BigInteger.valueOf(10_000))
            val treasury =
                InMemoryDomainTreasury(
