...
class RevocationDslTest {
    ...
    @Test
    fun `test createStatusList without issuer throws exception`() =
        runBlocking<Unit> {
    ...
    @Test
    fun `test revoke credential without credentialId throws exception`() =
        runBlocking<Unit> {
            assertFailsWith<IllegalStateException> {
                trustWeave.revoke {
                    statusList("list-id")
    ...
    @Test
    fun `test check revocation status`() =
        runBlocking<Unit> {
            val issuerDid = "did:key:issuer"
            val credentialId = "cred-123"
    ...
    @Test
    fun `test createStatusList with custom size`() =
        runBlocking<Unit> {
            val issuerDid = "did:key:issuer"
    ...
    @Test
    fun `test revoke credential without statusListId throws exception`() =
        runBlocking<Unit> {
            assertFailsWith<IllegalStateException> {
                trustWeave.revoke {
                    credential("cred-123")
    ...
    @Test
    fun `test getStatusList without statusListId throws exception`() =
        runBlocking<Unit> {
            assertFailsWith<IllegalStateException> {
