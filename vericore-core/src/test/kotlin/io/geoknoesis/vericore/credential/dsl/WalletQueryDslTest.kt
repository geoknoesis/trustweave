...
class WalletQueryDslTest {
    ...
    fun setup() = runBlocking {
        ...
        // Add tags if wallet supports organization
        if (wallet is io.geoknoesis.vericore.credential.wallet.CredentialOrganization) {
            wallet.tagCredential("cred-1", setOf("education", "degree"))
            wallet.tagCredential("cred-2", setOf("certification", "cloud"))
        }
    }
    ...
    fun `test queryEnhanced by collection`() = runBlocking {
        if (wallet is io.geoknoesis.vericore.credential.wallet.CredentialOrganization) {
            val collectionId = wallet.createCollection("Test Collection")
