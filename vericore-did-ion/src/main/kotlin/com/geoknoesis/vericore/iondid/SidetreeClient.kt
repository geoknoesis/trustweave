...
class SidetreeClient(
    ...
    suspend fun submitOperation(operation: JsonObject): OperationResponse = withContext(Dispatchers.IO) {
        try {
            ...
            val json = Json.encodeToString(operation)
    ...
    private fun vmToJsonObject(vm: com.geoknoesis.vericore.did.VerificationMethodRef): JsonObject {
        return buildJsonObject {
            put("id", vm.id)
            put("type", vm.type)
    ...
    private fun serviceToJsonObject(service: com.geoknoesis.vericore.did.Service): JsonObject {
        return buildJsonObject {
            put("id", service.id)
