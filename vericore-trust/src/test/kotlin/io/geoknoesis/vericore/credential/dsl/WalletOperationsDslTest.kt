...
class WalletOperationsDslTest {
    ...
    fun `test organize on non-organization wallet throws exception`() = runBlocking {
        ...
        val basicWallet = object : io.geoknoesis.vericore.credential.wallet.Wallet {
            ...
            override suspend fun list(filter: io.geoknoesis.vericore.credential.wallet.CredentialFilter?): List<VerifiableCredential> = emptyList()
            override suspend fun delete(credentialId: String) = false
            override suspend fun query(query: io.geoknoesis.vericore.credential.wallet.CredentialQueryBuilder.() -> Unit): List<VerifiableCredential> = emptyList()
        }
