...
class WalletStatisticsTest {
     ...
     */
    private inner class FixedWallet(
        private val creds: List<VerifiableCredential>,
    ) : Wallet {
    ...
    @Test
    fun `empty wallet returns all-zero statistics`() =
        runTest {
    ...
    @Test
    fun `totalCredentials counts all credentials`() =
        runTest {
    ...
    @Test
    fun `validCredentials counts unexpired non-revoked credentials with proof`() =
        runTest {
            val creds =
                listOf(
    ...
    @Test
    fun `expiredCredentials counts only past-expiration credentials`() =
        runTest {
            val creds =
                listOf(
    ...
    @Test
    fun `status references are unknown rather than revoked`() =
        runTest {
            val creds =
                listOf(
            ...
            val stats = wallet.getStatistics()
            assertEquals(0, stats.revokedCredentials)
            assertEquals(2, stats.unknownStatusCredentials)
        }

    // ── without CredentialOrganization capabilities ───────────────────────────

    @Test
    fun `collectionsCount is 0 when wallet does not implement CredentialCollections`() =
        runTest {
            val wallet = FixedWallet(emptyList())
            val stats = wallet.getStatistics()
            assertEquals(0, stats.collectionsCount)
    ...
    @Test
    fun `tagsCount is 0 when wallet does not implement CredentialTagging`() =
        runTest {
            val wallet = FixedWallet(emptyList())
            val stats = wallet.getStatistics()
            assertEquals(0, stats.tagsCount)
